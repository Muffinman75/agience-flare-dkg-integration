...
> [!WARNING]
> DKG v10 is release-candidate software on testnet. Expect breaking changes between
> releases. Use this integration for testing, demos, and governed authoring workflows,
> not for production-critical knowledge. Verifiable Memory (`vm-publish`) requires an
> on-chain registered Context Graph, gas + TRAC, and a reliable RPC. Content published
> to Verifiable Memory is irreversible — you are the Publisher under the DKG V10 Terms.
...
- **FLARE** — confidential retrieval boundary (enforced in the FLARE Index / Agience Core forks): AES-256-GCM encrypted vector search with Shamir K-of-M threshold oracle, Ed25519 signed grant ledger, light-cone graph authorization. 101 tests, 95.6% recall vs plaintext FAISS. This package consumes the FLARE policy result; the cryptographic implementation lives in the parent platform repositories.
...
- **MCP stdio server** (`agience-dkg-mcp`) — exposes `agience_wm_write`, `agience_share` (formerly `agience_promote`, still accepted), `agience_search` tools for Claude Desktop, Cursor, Claude Code, OpenClaw, Hermes, and any MCP-capable agent or host. Talks to either transport.
...
- Shares eligible assets to **Shared Memory** (SHARE) — daemon: `POST /api/knowledge-assets/{name}/swm/share` (the v10.0.1 operation historically called `promote`); MCP: `dkg-create` with `privacy=public`. Curator-authority, never automatic.
...
| **Sensitive content** | Omitted or manually redacted | FLARE-encrypted at the Agience Core / FLARE Index boundary (AES-256-GCM per-cell, Shamir threshold oracle key issuance); only derived projections reach DKG |
...
To target an MCP-fronted DKG node instead, add `"DKG_TRANSPORT": "mcp"`, set `"DKG_BASE_URL": "http://localhost:8083"`, and add `"DKG_TOKEN": "<mcp-bearer>"`. (The default `8083` matches the reference `dkg-node/apps/agent` MCP server used in our tests; if you are running the DKG Edge Node, point at its Agent Web UI/API port, typically `8081`.)

This exposes three tools: `agience_wm_write`, `agience_share`, `agience_search` (`agience_promote` is still accepted as an alias).
...
agience-dkg share <turnUri-from-above> --context-graph-id agience-demo
...
Speaks to a DKG node's `/mcp` endpoint (e.g. `dkg-node/apps/agent` or a node fronted by `dkg mcp setup`). Set `DKG_TRANSPORT=mcp`, `DKG_BASE_URL=http://localhost:8083`, and `DKG_TOKEN=<mcp-bearer>` in your `.env`, or pass `--transport mcp --base-url ... --token ...` per command. If you are using the DKG Edge Node, its Agent Web UI/API port is typically `8081`; adjust `DKG_BASE_URL` accordingly.
...
package/                Python package source (agience_dkg_integration)
  src/
    agience_dkg_integration/
      ...
      cli.py            agience-dkg CLI (wm-write, share, promote-alias, vm-publish, search; --transport switch)
  tests/
